        ...
        for column in Results:
            for node in Nodestable['node']:
                if node in column:
                    ...
                    #get number or id to match H node to Flow node from nodes table
                    noderow = Nodestable.loc[Nodestable['node']== node].index.values
                    noderownumber = noderow[0]
                    node_id_no = Nodestable.at[noderownumber,'node_no']
                    Node_number.append(node_id_no)
                    #supporting
                    #downloaded
                    #changed
                    #for upload
                    ...
                    for flcolumn in flResults:
                        for link in Nodestable['Link']:
                            if link in flcolumn:
                                ...
                                linkrow = Nodestable.loc[Nodestable['Link']== link].index.values
                                linkrownumber = linkrow[0]
                                link_id_no = Nodestable.at[linkrownumber,'node_no']
                                link_number.append(link_id_no)                              
